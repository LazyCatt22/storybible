...
3. **The Inquisition (religious enforcement)** — mandate: every world and person accepts the Quiet Lord and NO other god. Hunts heresy, apostasy, the alien-worship / Voice-cults, and the religious face of sedition (to reject the order is apostasy). **Answers to SAA + DEVANSH ONLY — NOT the Sevhar, NOT the Dukhar (corrected 2026-08-10); it stands OUTSIDE the pillar government.** The TEMPLE leans on it constantly as its doctrinal sword but can only ASK, not command. Its investigative wing is the likely home of Vesren Drath. (Full duties + hierarchy + the Voice-cults locked below — see "Inquisition ranks: duties, hierarchy & the Voice-cults".)
...
The apex of the Inquisition — answers to Saa and Devansh only (see command-line correction below). Rank read at a glance: **the one BARE FACE in the whole order + the single gold thread = the Hand.**
...
**COMMAND LINE (corrected).** The Inquisition stands OUTSIDE the pillar government. The Hand answers to **SAA + DEVANSH only** — not the Sevhar, not the Dukhar. Pillars/Temple can ASK, never command. (Supersedes the earlier "Saa + the Sevhar".) This is the source of the order's power and dread. The other four security layers sit under the Sevhar; the Inquisition does not.
...
- **THE HAND (head; SAA + DEVANSH only).** Commands the whole order; appoints/removes/unmakes Judges + Head Judges; SETS DOCTRINE (what heresy is, what Voice-infection looks like, how to hunt it — the single standard); holds the GRAVEST cases (mass outbreaks; taint reaching the Sevhar/Dukhar/Temple; a Judge gone heretic); can seal up to a WORLD-CHARTER; keeps the master Register; sole contact with Saa (temple-node link, and via her Devansh); the only one who can turn the sword on the powerful (governor/senator/pillar-head) and only on Saa's/Devansh's word — the leash AND the blade; guards the setting's deepest secret about the Voice/aliens/the demonic side.
