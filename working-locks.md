...
- **Now in the bible as their own entries** (2026-08-09): `sevhar` and `dukhar`, placed in render order right after `velakhar` (before `valkry`), `parents:['velakhar']`. The velakhar-places landmark now points to the Dukhar entry. Still to design: the Dukhar Hall interior + the cinematic set-pieces below; a Sevhar Chamber design; exact tiered-vote weights.
